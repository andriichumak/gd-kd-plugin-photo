...
2.  Develop your plugin code in `src/dp_kd_plugin_photo`

    The `src/dp_kd_plugin_photo/Plugin.tsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_kd_plugin_photo`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_dp_kd_plugin_photo.js"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_kd_plugin_photo](./src/dp_kd_plugin_photo/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_kd_plugin_photo](./src/dp_kd_plugin_photo) directory.

The [src/dp_kd_plugin_photo\_engine](./src/dp_kd_plugin_photo_engine) and [src/dp_kd_plugin_photo\_entry](./src/dp_kd_plugin_photo_entry) directories contain essential plugin boilerplate.
